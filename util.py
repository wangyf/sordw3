...
def prune( d, pattern=None, types=None ):
    ...
    # for k in d.keys():   In Python 2.x calling keys 
    # makes a copy of the key that you can iterate over 
    # while modifying the dict:  
    # Note that this doesn't work in Python 3.x because 
    # keys returns an iterator instead of a list.
    # Another way is to use list to force a copy of the 
    # keys to be made. This one also works in Python 3.x:
...
def load( fd, d=None, prune_pattern=None, prune_types=None ):
    ...
    """
    # if type( fd ) is not file: #python 2.7
    if not isinstance(fd,io.IOBase):
        fd = open( os.path.expanduser( fd ) )
    if d == None:
        d = {}
    # exec fd in d #python 2
    exec(fd.read(), d)
    prune( d, prune_pattern, prune_types )
    return namespace( d )
...
def makeeach(compiler, object_, source):
    import glob, difflib
    object_ = os.path.expanduser( object_ )
    source = tuple( os.path.expanduser( f.replace('.f90','') ) for f in source if f )
    compiler = compiler[:-1]
    ...
    state = []
    for f in source:
        command = compiler + ('-c',f+'.f90')
        state += [ ' '.join( command ) + '\n' ]
        state += open( f+'.f90' ).readlines()
    command = compiler + tuple( os.path.expanduser( f+'.o') for f in source ) + ('-o',object_)
    state += [ ' '.join( command ) + '\n']
    compile_ = True
