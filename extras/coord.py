...
def source_tensors( R ):
    ...
    strike, dip, normal = np.array([x for x in R.tolist()])
    del( R )
    strike = 0.5 * np.array([
    ...
    normal = np.array(normal) * np.array(normal)
    return np.array( [strike, dip, normal] )


def source_potency_tensors( R ):
    strike, dip, normal = np.array([x for x in R.tolist()])
    del( R )
    fulltensor=np.array([ #order is 23 31 12 11 22 33
        0.5*np.array([strike[1]*normal[2]+strike[2]*normal[1], #strike contribution
             strike[2]*normal[0]+strike[0]*normal[2],
             strike[0]*normal[1]+strike[1]*normal[0],
             strike[0]*normal[0]+strike[0]*normal[0],
             strike[1]*normal[1]+strike[1]*normal[1],
             strike[2]*normal[2]+strike[2]*normal[2]]),

        0.5*np.array([dip[1]*normal[2]+dip[2]*normal[1], #dip contribution
             dip[2]*normal[0]+dip[0]*normal[2],
             dip[0]*normal[1]+dip[1]*normal[0],
             dip[0]*normal[0]+dip[0]*normal[0],
             dip[1]*normal[1]+dip[1]*normal[1],
             dip[2]*normal[2]+dip[2]*normal[2]]), 
             
        0.5*np.array([normal[1]*normal[2]+normal[2]*normal[1], #normal contribution
             normal[2]*normal[0]+normal[0]*normal[2],
