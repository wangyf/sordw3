...
if ( eplasticity == 'plastic' ) then
    ...
    case('DP2')
        ...
        call fieldio( '<', 'phi', phi )
        phi = atan( phi )
        call fieldio( '<', 'plb', plb )
        call fieldio( '<', 'plh', plh )
    end select
end if
...
if ( eplasticity == 'plastic' ) then
    select case(plmodel)
    case('DP1')
        call set_halo( mco,  1e20, i1, i2 )
        call set_halo( phi,  0.0, i1, i2 )
    case('DP2')
        call set_halo( plb,  0.0, i1, i2 )
        call set_halo( plh,  0.0, i1, i2 )
        call set_halo( mco,  1e20, i1, i2 )
        call set_halo( phi,  0.0, i1, i2 )
    end select
end if
...
if ( eplasticity == 'plastic' ) then
    ...
    case('DP2')
        call scalar_swap_halo( mco, nhalo )
        call scalar_swap_halo( phi, nhalo )
        call scalar_swap_halo( plb, nhalo )
        call scalar_swap_halo( plh, nhalo )
    end select
end if
