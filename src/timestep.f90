...
use m_fieldio
use m_stats
use m_collective,only : vector_swap_halo
use m_thermpres, only : record_svel_ts
! Status
if ( master ) then
    if ( verb ) then
...
if (friction == 'rateandstate'.or. &
    friction == 'thermalpressurization') call vector_swap_halo( vv, nhalo )
...
if ( ifn /= 0 ) then
    ...
    t2(:,:,:,1) = lamf(:,:,:)*area(:,:,:)*sum(t1*nhat,4) + &
                     2*muf(:,:,:)*area(:,:,:)*t1(:,:,:,1)*nhat(:,:,:,1)
    t2(:,:,:,2) = lamf(:,:,:)*area(:,:,:)*sum(t1*nhat,4) + &
                     2*muf(:,:,:)*area(:,:,:)*t1(:,:,:,2)*nhat(:,:,:,2)  
    t2(:,:,:,3) = lamf(:,:,:)*area(:,:,:)*sum(t1*nhat,4) + &
    ...
    call fieldio( '>', 'mr11',  t2(:,:,:,1) )
    call fieldio( '>', 'mr22',  t2(:,:,:,2) )
    call fieldio( '>', 'mr33',  t2(:,:,:,3) ) 
    call fieldio( '>', 'mr23',  t3(:,:,:,1) )
    call fieldio( '>', 'mr31',  t3(:,:,:,2) )
    call fieldio( '>', 'mr12',  t3(:,:,:,3) )                
